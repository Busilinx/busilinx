...
<?php print t('You have successfully completed the ' . ($node->title) . ' form on @date', array('@date' => format_date(time(), 'small'))) ?>

<?php print t('Please visit <a href="http://gsl.tv">http://gsl.tv</a> to keep up with the latest information on the summit')?>
...
<?php print t('The details of your registration are') ?>:
...
<?php print t('Please contact us if they are incorrect or visit the following link to correct them yourself:') ?>

<?php print url('node/'. $node->nid .'/submission/'. $sid, array('absolute' => TRUE)) ?>

<?php print t('We look forward to welcoming you at the summit')?>
